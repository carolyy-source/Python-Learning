...
<template>
  <div class="relative bg-gray-900 h-[500px]">
    ...
    <div
      class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col justify-center"
    >
      <h1
        class="text-4xl md:text-5xl font-bold text-white mb-8 text-center md:text-left drop-shadow-lg"
      >
        開啟你的旅程
      </h1>
      ...
      <div class="bg-white rounded-lg shadow-xl p-2 max-w-4xl">
        <!-- Tabs -->
        <div
          class="flex space-x-1 border-b border-gray-200 mb-4 overflow-x-auto pb-2"
        >
          <button
            v-for="tab in tabs"
            :key="tab"
        ...
        <div class="p-4 grid grid-cols-1 md:grid-cols-12 gap-4">
          ...
          <div class="md:col-span-3">
            ...
            <input
              type="text"
              :placeholder="
                activeTab === '租車' ? '經濟型, SUV...' : '1 間房, 2 位成人'
              "
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-primary focus:border-primary outline-none"
            />
          </div>
          <div class="md:col-span-1 flex items-end">
            <button
              @click="handleSearch"
              class="w-full bg-primary hover:bg-primary-hover text-white font-bold py-3 px-4 rounded-md transition-colors flex items-center justify-center"
            >
              <svg
                class="w-6 h-6"
                fill="none"
